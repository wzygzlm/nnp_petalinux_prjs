...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"if run sd_uEnvtxt_existence_test; then" \ 
		...
		"run importbootenv; \0" \ 
	"sdboot=echo boot Petalinux; fatload mmc 0 ${netstart} ${kernel_img} && bootm ${netstart}\0" \ 
	"autoload=no\0" \ 
	...
	"sdboot_fb=echo boot Petalinux (fallback); fatload mmc 0 ${netstart} ${fbkernel_img} && bootm ${netstart}\0" \
""
